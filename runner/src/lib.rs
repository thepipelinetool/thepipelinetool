...
pub trait DefRunner {
    fn enqueue_run(&mut self, dag_name: &str, dag_hash: &str) -> usize;
...
impl<U: Runner> DefRunner for U {
    ...
    fn enqueue_run(&mut self, dag_name: &str, dag_hash: &str) -> usize {
        let dag_run_id = self.create_new_run(dag_name, dag_hash);
    ...
    fn run_dag_local(&mut self, max_threads: usize) {
        ...
        let dag_run_id = &self.enqueue_run(&self.get_dag_name(), &hash);
